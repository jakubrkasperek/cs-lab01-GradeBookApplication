    ...
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            switch (type)
            {
                case ("standard"):
                    {
                        var newStandardGradeBook = new StandardGradeBook(name, bool.Parse(isWeighted));
                        Console.WriteLine("Created gradebook {0}.", name);
                        GradeBookUserInterface.CommandLoop(newStandardGradeBook);
                        return;
                    }
                case ("ranked"):
                    {
                        var newRankedGradeBook = new RankedGradeBook(name, bool.Parse(isWeighted));
                        Console.WriteLine("Created gradebook {0}.", name);
                        GradeBookUserInterface.CommandLoop(newRankedGradeBook);
                        return;
                    }
                default:
                    {
